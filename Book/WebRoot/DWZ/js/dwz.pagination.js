...
(function($){
		...
		pagination: function(opts){
			...
			return this.each(function(){
				...
				if (pc.hasPrev()){
					...
					_bindEvent($prev, pc.getCurrentPage()-1, pc.targetType(), pc.rel(),pc.contains_search());
					_bindEvent($first, 1, pc.targetType(), pc.rel(),pc.contains_search());
				} else {
				...
				if (pc.hasNext()) {
					...
					_bindEvent($next, pc.getCurrentPage()+1, pc.targetType(), pc.rel(),pc.contains_search());
					_bindEvent($last, pc.numPages(), pc.targetType(), pc.rel(),pc.contains_search());
				} else {
					$next.add($last).addClass("disabled").find(">a").hide();
				}
	
				$this.find(setting.nums$).each(function(i){
					_bindEvent($(this), i+interval.start, pc.targetType(), pc.rel(),pc.contains_search());
				});
				$this.find(setting.jumpto$).each(function(){
					...
					$button.click(function(event){
						...
						if (pageNum && pageNum.isPositiveInteger() && pc.contains_search()=='0') {
							dwzPageBreak({targetType:pc.targetType(), rel:pc.rel(), data: {pageNum:pageNum}});
						}
						else if (pageNum && pageNum.isPositiveInteger() && pc.contains_search()=='1') {
							dwzPageBreakSearch({targetType:pc.targetType(), rel:pc.rel(), data: {pageNum:pageNum}});
						}
					});
			...
			function _bindEvent($target, pageNum, targetType, rel,contains_search){
				$target.bind("click", {pageNum:pageNum}, function(event){
					if(contains_search=='0'){
						dwzPageBreak({targetType:targetType, rel:rel, data:{pageNum:event.data.pageNum}});
					}else if(contains_search=='1'){
						dwzPageBreakSearch({targetType:targetType, rel:rel, data:{pageNum:event.data.pageNum}});
					}
	...
	$.extend(Pagination.prototype, {
		...
		contains_search:function(){return this.opts.contains_search},
		numPages:function() {
