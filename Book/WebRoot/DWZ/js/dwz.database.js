			...
			return this.each(function(){
				...
				$table.find("tr:first th[type]").each(function(i){
					...
					var field = {
						...
						others:$th.attr("others") || "",  //添加others
						others1:$th.attr("others1") || ""  //添加others1
					};
					fields.push(field);
				});
				...
				if (addButTxt) {
					...
					/**
					if(saveButTxt.length>0){
						var $saveBut = $('<div class="button"><div class="buttonContent"><button type="button">'+saveButTxt+'</button></div></div>').insertBefore($table);
					}
					*/
			...
			function tdHtml(field){
				...
				switch(field.type){
					...
					case 'lookupnew': //添加
						...
						html = '<input type="hidden" name="'+field.lookupGroup+'.'+field.lookupPk+suffix+'"/>'
							+ ' <textarea cols ="22" rows = "3" name="'+field.name+'"'+suggestFrag+' lookupPk="'+field.lookupPk+'" size="'+field.size+'" class="'+field.fieldClass+'"/>'
							+ '<a class="btnLook" href="'+field.lookupUrl+'" lookupGroup="'+field.lookupGroup+'" '+suggestFrag+' lookupPk="'+field.lookupPk+'" title="查找带回">查找带回</a>';
						break;
					case 'attach':
						html = '<input type="hidden" name="'+field.lookupGroup+'.'+field.lookupPk+suffix+'"/>'
					...
					case 'enum':
						...
						break;
					case 'date':
						html = '<input type="text" name="'+field.name+'" value="'+field.defaultVal+'" class="date '+field.fieldClass+'" format="'+field.patternDate+'" size="'+field.size+'"'+'id='+field.id+'/>'
							+'<a class="inputDateButton" href="javascript:void(0)">选择</a>';
						break;
					case 'hidden'://添加
						html = '<input type="hidden" name="'+field.name+'" id="'+field.id+'" value="'+field.defaultVal+'" size="'+field.size+'" class="'+field.fieldClass+'" '+attrFrag+'/>';
						break;	
					case 'button': //添加
						html = ' <div class="buttonActive"> <div class="buttonContent"><button type="button" name="'+field.name+'" id="'+field.id+'"   size="'+field.size+'"  '+attrFrag+' '+field.others+'>'+field.defaultVal+'</button></div></div>'; 
						break;	
				    case 'suggest': //添加
						var suggestFrag = '';
						if (field.suggestFields) {
						...
						html = '<input type="hidden" name="'+field.lookupGroup+'.'+field.lookupPk+suffix+'"'+' '+field.others+'/>'
							+ '<input type="text" name="'+field.name+'"'+suggestFrag+' '+field.others1+' lookupPk="'+field.lookupPk+'" size="'+field.size+'" class="'+field.fieldClass+'" value="'+field.defaultVal+'"'+field.others+'/>';
						break;			
					case 'text': //添加
					  html = '<input type="text" name="'+field.name+'" value="'+field.defaultVal+'" size="'+field.size+'" class="'+field.fieldClass+'" '+attrFrag+' '+field.others+'/>';
						break;
					default:
