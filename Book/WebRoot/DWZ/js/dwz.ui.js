 ...
 */
function initEnv() {//初始化布局,初始化taskBar,初始化navTab
	$("body").append(DWZ.frag["dwzFrag"]);//显示遮罩“数据加载中，请稍等...”

	if ( $.browser.msie && /6.0/.test(navigator.userAgent) ) {//如果你给链接、按钮用CSS sprites作为背景，你可能会发现在IE6下会有背景图闪烁的现象。造成这个的原因是由于IE6没有将背景图缓存，每次触发hover的时候都会重新加载，可以用JavaScript设置IE6缓存这些图片
		try {
	...
	$(window).resize(function(){//window大小调整事件触发
		initLayout();//重新调整布局宽高
		$(this).trigger(DWZ.eventType.resizeGrid);//...
	});

	var ajaxbg = $("#background,#progressBar");//布局准备完毕后隐藏遮罩
	ajaxbg.hide();
	$(document).ajaxStart(function(){//让ajax开始的时候启用遮罩
		ajaxbg.show();
	}).ajaxStop(function(){//让ajax结束的时候隐藏遮罩
		ajaxbg.hide();
	});
	
	$("#leftside").jBar({minW:150, maxW:700});//设置左菜单栏的最小宽度与最大宽度
	
	if ($.taskBar) $.taskBar.init();//初始底下任务栏
	navTab.init();//初始navTab(包括注册tab栏的切换和关闭事件，右键菜单，左右两边按钮样式等等)
	if ($.fn.switchEnv) $("#switchEnvBox").switchEnv();//切换城市
	if ($.fn.navMenu) $("#navMenu").navMenu();//横向菜单处理
...
function initLayout(){//初始首页各布局大小
	...
	$("#taskbar").css({top: iContentH + $("#header").height() + 5, width:$(window).width()});//设置下方taskbar栏高度和宽度
}
...
function initUI(_box){
	...
	$("div.pagination", $p).each(function(){
		...
		$this.pagination({
			...
			rel:$this.attr("rel"),
			contains_search:$this.attr("contains_search"),
