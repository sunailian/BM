...
(function($){
	...
	$.taskBar = {
		...
		init:function(options) {
			...
			if (this._taskBar.size() == 0) {//初始化任务栏
				this._taskBar = $(DWZ.frag["taskbar"]).appendTo($("#layout"));//将底部任务栏碎片加载到页面，默认隐藏
				
				this._taskBar.find(".taskbarLeft").hoverClass("taskbarLeftHover");//追加任务栏的左箭头的鼠标悬浮事件
				this._taskBar.find(".taskbarRight").hoverClass("taskbarRightHover");//追加任务栏的左箭头的鼠标悬浮事件
			}
			this._taskBox = this._taskBar.find(this._op.taskBox);//任务容器
			this._taskList = this._taskBox.find(">ul");
			this._prevBut = this._taskBar.find(this._op.prevBut);//向左移元素
			this._nextBut = this._taskBar.find(this._op.nextBut);//向右移元素
			this._prevBut.click(function(e){$this.scrollLeft()});//左移事件
			this._nextBut.click(function(e){$this.scrollRight()});//右移事件
		...
		_visibleStart: function(){
			...
			var jTasks = this._getTasks();//所有任务li
