...
var navTab = {
	...
	init: function(options){//初始navTab
		...
		this._prevBut.click(function(event) {$this._scrollPrev()});//tab前移事件
		this._nextBut.click(function(event) {$this._scrollNext()});//tab后移事件
		...
		this._contextmenu(this._tabBox);//注册右键菜单
		this._contextmenu(this._getTabs());//注册右键菜单
		
		this._init();//初始化各标签的切换和关闭事件
		this._ctrlScrollBut();//处理标签栏左边两边的样式
	},
	_init: function(){//初始化各标签的切换和关闭事件
		var $this = this;
		this._getTabs().each(function(iTabIndex){//取消各tab的默认点击事件，绑定新的点击事件为“切换标签”
			$(this).unbind("click").click(function(event){
				$this._switchTab(iTabIndex);//切换标签
			});
			$(this).find(navTab._op.close$).unbind("click").click(function(){//取消各tab里关闭链接的点击事件，绑定新的点击事件为“关闭标签”
				$this._closeTab(iTabIndex);//关闭标签
			});
		});
		this._getMoreLi().each(function(iTabIndex){//更多里也取消各链接的默认点击事件，绑定新的点击事件为“切换标签”
			$(this).find(">a").unbind("click").click(function(event){
				$this._switchTab(iTabIndex);
			});
		});

		this._switchTab(this._currentIndex);//立即切换到当前tab
	},
	...
	getPanel: function(tabid){//根据tabid获取对应的面板
		var index = this._indexTabId(tabid);
	...
	_indexTabId: function(tabid){//根据tabid获取tab索引
		if (!tabid) return -1;
		var iOpenIndex = -1;
	...
	_ctrlScrollBut: function(){
		var iW = this._tabsW(this._getTabs());
		if (this._getScrollBarW() > iW){//标签栏宽度大于标签的宽度，则移除左右按钮和空间
			this._prevBut.hide();
			this._nextBut.hide();
	...
	_switchTab: function(iTabIndex){//传navTab的索引，切换到对应的navTab
		var $tab = this._getTabs().removeClass("selected").eq(iTabIndex).addClass("selected");
		this._getPanels().hide().eq(iTabIndex).show();//显示对应的面板

		this._getMoreLi().removeClass("selected").eq(iTabIndex).addClass("selected");
		this._currentIndex = iTabIndex;//将标签档期那索引记录下来
		
		this._scrollCurrent();//自动滚动标签
		this._reload($tab);//重载
	},
			
	_closeTab: function(index, openTabid){

		this._getTabs().eq(index).remove();//移除标签
		this._getPanels().eq(index).trigger(DWZ.eventType.pageClear).remove();//移除面板
		this._getMoreLi().eq(index).remove();//移除更多标签里的项
		if (this._currentIndex >= index) this._currentIndex--;//当前标签如果在关闭标签后面，则当前标签索引要减一
	...
	_reload: function($tab, flag){//传标签来重载对应面板数据
		flag = flag || $tab.data("reloadFlag");
		var url = $tab.attr("url");
		if (flag && url) {//如果需要重载
			$tab.data("reloadFlag", null);
			var $panel = this.getPanel($tab.attr("tabid"));//得到对应的面板
			
			if ($tab.hasClass("external")){//在iframe中打开
	...
	reload: function(url, options){
		...
		if ($panel){
			if (!url) {
				url = $tab.attr("url");//如果本次刷新没传url，则设置url值为tab上的url(即打开标签的url)
			}
			if (url) {
				if ($tab.hasClass("external")) {//iframe中打开
					navTab.openExternal(url, $panel);
				} else {
	...
	openExternal:function(url, $panel){//在iframe中打开页面
		var ih = navTab._panelBox.height();
		$panel.html(DWZ.frag["externalFrag"].replaceAll("{url}", url).replaceAll("{height}", ih+"px"));//动态把缓存的iframe碎片中的url和height属性替换掉
	},
	/**
	 * 
	 * @param {Object} tabid
