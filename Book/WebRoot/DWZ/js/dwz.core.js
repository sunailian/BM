...
var DWZ = {
	...
	init:function(pageFrag, options){//将被首页调用，初始化DWZ
		var op = $.extend({
				loginUrl:"login.html", loginTitle:null, callback:null, debug:false, 
				statusCode:{}
			}, options);
		this._set.loginUrl = op.loginUrl;//设置登录页
		this._set.loginTitle = op.loginTitle;//设置登录标题
		this._set.debug = op.debug;//设置调试模式
		$.extend(DWZ.statusCode, op.statusCode);//设置状态编码
		$.extend(DWZ.pageInfo, op.pageInfo);//设置分页信息
		
		jQuery.ajax({//加载xml,将页面碎片缓存于DWZ的frag变量中,将提示信息缓存于DWZ的_msg变量中
			type:'GET',
			url:pageFrag,
			dataType:'xml',
			timeout: 50000,
			...
			success: function(xml){
				...
				if (jQuery.isFunction(op.callback)) op.callback();//缓存变量完毕后，调用回调函数
			}
		});
...
(function($){
	...
	$.fn.extend({
		 ...
		 */
		ajaxUrl: function(op){//让调用的元素加载返回结果
			var $this = $(this);
